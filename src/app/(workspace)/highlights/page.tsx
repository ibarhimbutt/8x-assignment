...
export default function HighlightsPage() {
  ...
  return (
    <div>
      <p className="font-mono text-[11px] tracking-[0.16em] text-brass uppercase">Moments</p>
      <h1 className="mt-2 font-display text-4xl italic">Highlights</h1>
      ...
      <ul className="mt-8 grid gap-3 sm:grid-cols-2">
        {rows.map((h) => (
          <li key={`${h.meetingId}-${h.id}`} className="rounded-2xl border border-line p-4">
            <p className="font-mono text-[10px] uppercase text-brass">{h.type}</p>
            <h2 className="mt-1 font-display text-[22px] italic">{h.title}</h2>
            ...
            <Link
              ...
              className="mt-3 inline-block text-[13px] text-brass"
            >
              Jump to moment
            </Link>
          </li>
