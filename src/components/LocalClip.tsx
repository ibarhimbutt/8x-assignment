...
export function LocalClip({
  ...
  if (!meeting || highlight === null) {
    return (
      <div className="mx-auto max-w-xl px-6 py-20">
        <p className="font-display text-3xl italic">This clip isn’t here.</p>
        <Link href="/" className="mt-4 inline-block text-brass">
          Open Quorum
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      ...
      <h1 className="mt-4 font-display text-4xl italic leading-tight">{highlight.title}</h1>
      <p className="mt-2 font-mono text-[12px] text-paper-dim">
        {meeting.title} · {formatClock(highlight.start)} · {highlight.type}
      </p>
      {utterance && (
        <blockquote className="mt-10 border-l-2 border-brass pl-5 font-display text-[26px] leading-snug italic">
          “{utterance.text}”
          {speaker && (
      ...
      <Link
        ...
        className="mt-10 inline-flex rounded-full bg-brass px-4 py-2 text-[13px] font-medium text-ink"
      >
